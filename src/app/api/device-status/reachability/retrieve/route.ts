...
/**
 * POST /device-status/reachability/retrieve
 * Check if a device is reachable - Returns 200 OK per CAMARA spec
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await adapter.getReachabilityStatus({
    ...
    console.error('Reachability status error:', error);
    const status = error.response?.status || 500;
    return errorResponse(
      status,
      mapStatusToErrorCode(status),
      error.message || 'Failed to get reachability status',
      correlator
    );
  }
}
