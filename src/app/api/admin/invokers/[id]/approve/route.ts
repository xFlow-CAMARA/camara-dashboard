...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // Authenticated identity always wins over whatever the client sent.
    const body = { ...incoming, approved_by: session.user.email };

    const headers: Record<string, string> = { 'Content-Type': 'application/json' };
    if (ADMIN_API_KEY) headers['X-Admin-Api-Key'] = ADMIN_API_KEY;

    const r = await fetch(`${ONBOARDING_URL}/admin/invokers/${params.id}/approve`, {
      method: 'POST',
      headers,
