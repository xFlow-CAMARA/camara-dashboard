...
/**
 * POST /api/sim-swap/retrieve-date
 ...
 * 
 * Response:
 * - latestSimChange: string | null (ISO 8601 timestamp or null if not available)
 * - monitoredPeriod?: number (days monitored, provided when latestSimChange is null)
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${TF_SDK_BASE_URL}/sim-swap/vwip/retrieve-date`, {
      method: 'POST',
      headers: {
    ...
    console.error('SIM Swap retrieve-date error:', error);
    return NextResponse.json(
